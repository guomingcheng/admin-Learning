...
contract Market is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public defaultFee = 100; // 默认收取百分之 1
    ...
    bool placementRewardEnabled = false; //为将NFT代币投放市场启用奖励
    ...
    mapping(address => bool) public nftBlacklist; //在黑名单上添加令牌
    mapping(address => bool) public nftForAccrualRB; //添加RobiBoost应计的代币
    /**
     * 结算代币，只有这里添加代币才能用于交易 NFT
     **/
    mapping(address => bool) public dealTokensWhitelist;
    mapping(address => uint256) public userFee; //用户交易费。如果为零，则默认为费用
    mapping(address => uint256) public tokensCount; //用户的代币销售数量：用户=>计数
