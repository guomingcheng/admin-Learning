...
contract BSWToken is BEP20('Biswap', 'BSW') {
    ...
    function mint(address _to, uint256 _amount) public onlyMinter returns(bool) {
        _mint(_to, _amount);
        // 转移委托，将 amount 数额的票数转移到 to 地址的委托人上
        // 这里看的话，委托人也是 0 地址，那就表示如果 to 地址不设置委托人，那么就没有那个地址能替代 to 地址投票
   ...
   /**
    * 转移当前 msg.sender 的委托人
    *
    * 只有成为委托人才能投票，但是可以把委托人设置成自己，这样就有了投票权了
    */
    ...
    /**
     * 获取 account 在 blockNumber 区块前持有的投票数
     * 这个投票机制，只要在当前提案的区块前持有投票数，就算委托人在后面区快发生了更改，也不会影响该提案的投票。
     * 委托人持有的 10000 投票数在于提案前，就会对该提案拥有 10000 投票权。并不会因为后面区块投票数变更而影响
     * 
     * 块号必须是已完成的块，否则此功能将还原以防止出现错误信息
        ...
        returns (uint256)
    {
        ...
        // 如果委托人最后票数变更的区块，小于 blockNumber 区块
        // 就直接返回持有的票数即可
        ...
        // 那么就表示当前委托人并不在 blockNumber 区块前持有投票数，返回 0 票数即可
    ...
    function _moveDelegates(address srcRep, address dstRep, uint256 amount) internal {
        ...
        if (srcRep != dstRep && amount > 0) {
            ...
            // srcRep 如果等于 0 地址，就表示被铸造方法产出的新票。这些票并没有委托过给别人，就不需要更改 srcRep 状态
            if (srcRep != address(0)) {
            ...
            // dstRep 如果等于 0 地址，那么就表示这些新的票被销毁掉了
            // 否择就更新状态
            if (dstRep != address(0)) {
